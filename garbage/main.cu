...
int store_pixels(char* filename, animated_gif* image)
{
    ...
    /* Initialize the new_image set of colors */
...
__global__ void sobel_filter_kernel(int* image, int* sobel, int width, int height) {
    ...
    if (j > 0 && j < height - 1 && k > 0 && k < width - 1) {
        ...
        float val = sqrt(deltaX * deltaX + deltaY * deltaY) / 4;
        if (val > 50) {
...
void apply_blur_filter(animated_gif* image, int size, int threshold)
{
    ...
    for (i = 0; i < image->n_images; i++) {
        ...
        /* Allocate array of new_image pixels */
        ...
        do {
            ...
            for (j = size; j < height / 10 - size; j++) {
                for (k = size; k < width - size; k++) {
                    ...
                    new_image[CONV(j, k, width)].r = t / ((2 * size + 1) * (2 * size + 1));
            ...
            for (j = 1; j < height - 1; j++) {
                for (k = 1; k < width - 1; k++) {
                    ...
                    diff = (new_image[CONV(j, k, width)].r - p[i][CONV(j, k, width)].r);
...
int* gif_to_flatten_array(const animated_gif* image, int num_images)
{
    ...
    int idx=0;
    int* array = (int*)malloc(nb_pixels * sizeof(int));
...
long long int* get_image_offsets(int* widths, int* heights, int n_images) {
    ...
    offsets = (long long int*)malloc((n_images + 1) * sizeof(long long int));
...
int main(int argc, char** argv)
{
    ...
    int* image_information = (int*)malloc(sizeof(int) * 3);
